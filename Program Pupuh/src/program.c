...
void input_data (struct Data_tersimpan_t **head, struct Padalingsa_t **root) {
	...
	do {	
		ulangi = 0;
		filename = get_string(128, "Masukkan nama file yang ingin diproses : "); 
		fp = fopen(filename, "r");
		if (fp == NULL) {
			print_n_pause("Tidak dapat membuka file %s :(", filename);
	...
	is_simpan_data = get_range(0, 2, "Simpan data ? (input 1 jika iya) : ");
	if (is_simpan_data)
